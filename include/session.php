...
class Session {
    ...
    function checkLogin() {
        ...
        if (isset($_SESSION['username']) && isset($_SESSION['userid']) && $_SESSION['username'] != GUEST_NAME) {
            ...
            if (isset($_SESSION['expire']) && $now > $_SESSION['expire']) {
                ...
                $database->removeActiveUser($_SESSION['username']);
                return false;
            }
    ...
    function login($subuser, $subpass, $subremember) {
        ...
        $field = "user"; //Use field name for password
        if (!$subpass) {
     ...
     */
    function register($subemail, $subpass, $subconf_pass, $subfirstname, $sublastname, $subphone) {
        global $database, $form, $mailer; //The database, form and mailer object
        $token  = $this->generateRandStr(16);
        $config = $database->getConfigs();
        
        /* Username/Email error checking */
        ...
        if (!$subemail || strlen($subemail = trim($subemail)) == 0) {
            ...
            else if ($database->usernameBanned($subemail)) {
        ...
        /* No errors, add the new account to the database */
            ...
            if ($database->addNewUser($subemail, $subpass, $token, $usersalt, $subfirstname, $sublastname, $subphone)) {
                /* Check Account activation setting and process accordingly. */
                
                /* E-mail Activation */
                if ($config['ACCOUNT_ACTIVATION'] == 2) {
                    $config = $database->getConfigs();
                    $mailer->sendActivation($subemail, $subemail, $subpass, $token, $config);
                    $successcode = 3;
                }
                /* Admin Activation */
                else if ($config['ACCOUNT_ACTIVATION'] == 3) {
                    $config = $database->getConfigs();
                    $mailer->adminActivation($subemail, $subemail, $subpass, $config);
                    $mailer->activateByAdmin($subemail, $subemail, $subpass, $token, $config);
                    $successcode = 4;
                }
                /* No Activation Needed but E-mail going out */
                else if ($config['EMAIL_WELCOME'] && $config['ACCOUNT_ACTIVATION'] == 1) {
                    $config = $database->getConfigs();
                    $mailer->sendWelcome($subemail, $subemail, $subpass, $config);
                    $successcode = 5;
                } else {
    ...
    function editConfigs($subsitename, $subsitedesc, $subemailfromname, $subadminemail, $subwebroot, $subhome_page, $subactivation, $submin_user_chars, $submax_user_chars, $submin_pass_chars, $submax_pass_chars, $subsend_welcome, $subenable_login_question, $sub_captcha, $sub_all_lowercase, $subuser_timeout, $subguest_timeout, $subcookie_expiry, $subcookie_path) {
        ...
        if ($subadminemail) {
            ...
            } else /* Check if valid email address using PHPs filter_var */ if (!filter_var($subadminemail, FILTER_VALIDATE_EMAIL)) {
                $form->setError($field, "* Email invalid");
            }
        }
        ...
        if ($subcookie_expiry) {
            ...
            } else if (!filter_var($subcookie_expiry, FILTER_VALIDATE_INT, array(
                    "options" => array(
                        "max_range" => 366
                    )
                ))) {
     ...
     * automatically.
     */
    function editAccount($subcurpass, $subnewpass, $subconfnewpass, $subemail, $userregion) {
        global $database, $form; //The database and form object
        /* New password entered */
        if ($subnewpass) {
            ...
            if (!$subcurpass) {
                ...
                if (strlen($subcurpass) < $config['min_pass_chars'] || !preg_match("/^([0-9a-z])+$/i", ($subcurpass = trim($subcurpass)))) {
        ...
        if ($userregion) {
            $database->updateUserField($this->username, "region", $userregion);
        }
    ...
    function addNewJob($job_id, $job_name) {
        global $database;
        $result = $database->addNewJob($job_id, $job_name);
        return $result;
    }

    function addNewSubhead($job_id, $subhead_name) {
        global $database;
        $result = $database->addNewSubhead($job_id, $subhead_name);
        return $result;
    }

    function addNewItem($item_code, $item_desc, $item_usage, $unit_measure) {
        global $database;
        $result = $database->addNewItem($item_code, $item_desc, $item_usage, $unit_measure);
        return $result;
    }

    function retrieveItem($item_code, $item_desc, $item_usage) {
        global $database;
        $result = $database->retrieveItem($item_code, $item_desc, $item_usage);
        return $result;
    }

    function addItemToJob($job_id, $subhead_id, $item_id, $quantity) {
        global $database;
        $result = $database->addItemToJob($job_id, $subhead_id, $item_id, $quantity);
        return $result;
    }

    function retrieveSubheadID($job_id, $subhead_name) {
        global $database;
        $result = $database->retrieveSubheadID($job_id, $subhead_name);
        return $result;
    }

    function updateJobItem($job_id, $subhead_id, $item_id, $quantity) {
        global $database;
        $result = $database->updateJobItem($job_id, $subhead_id, $item_id, $quantity);
        return $result;
    }

    function retrieveJobItem($job_id, $subhead_id, $item_id) {
        global $database;
        $result = $database->retrieveJobItem($job_id,$subhead_id,$item_id);
        return $result;
    }

    function updateAccess($access_id) {
        global $database;
        $result = $database->updateAccess($access_id);
        return $result;
    }
